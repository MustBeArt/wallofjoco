...
key = b'\x4d\x92\x91\x63\x8b\x4b\x46\xd0\x67\x8d\x67\x53\xa3\xc8\xbe\x7e'
...
def decrypt_short_cryptable(cryptable):
	...
	cleartext = bytes(a^b for a,b in zip(ciphertext, cipher.encrypt(iv)))
	return cleartext
...
def decrypt_cryptable(cryptable):
	...
	while len(ciphertext) > 0:
		block = bytes(a^b for a,b in zip(ciphertext, cipher.encrypt(iv)))
		cleartext += block
		iv = increment_ctr(iv)
...
def encrypt(cleartext):
	...
	while len(cleartext) > 0:
		block = bytes(a^b for a,b in zip(cleartext, cipher.encrypt(iv)))
		ciphertext += block
		iv = increment_ctr(iv)
